...
| 현재 기준선 | 새 V1 목표 | 전환 |
| --- | --- | --- |
| `ledger_months.budget_month` | `budget_periods.start_date/end_date` | 장부 기간 규칙으로 날짜 구간 생성 |
| 월 총예산·member allocation | 기간 전체·개인·공동·예비비 배분 | 기존 멤버 할당 보존, 공동 배분은 0으로 초기화 후 사용자 확인 |
| 사용자 생성 category group | 고정 system category group | 검증된 이름만 mapping하고 미매핑 소분류는 `기타` 대분류 아래 원래 이름으로 보존·보고 |
...
| payer 중심 거래 | budget allocation 차감 | 개인 장부는 소유자 할당, 공동 장부는 기존 payer의 개인 할당으로 임시 mapping |
...
공동 장부 거래의 차감 할당은 제품 의미를 자동으로 확정할 수 없습니다. 초기 backfill은 기존 결제자의 개인 할당으로 매핑하고, migration report와 첫 진입 확인 화면에서 사용자가 공동 예산 거래를 재분류할 수 있게 해야 합니다.

## 단계

### 1. 사전 점검

- 개인 장부가 없거나 둘 이상인 사용자 수
- 활성 멤버가 두 명을 초과하는 공동 장부 수
- owner가 없거나 여러 명인 장부 수
- 잘못된 category/type 참조와 orphan 거래 수
- 중복 장부 월, 마감 상태, 음수 금액 수
- pending 직접 초대와 30분보다 오래된 링크 수
- archived 장부와 settlement 지급 이력 수

사전 점검이 0을 요구하는 항목과 별도 격리가 가능한 항목을 migration report에 구분합니다.

- 추가 개인 장부는 삭제하지 않고 기본 장부가 아닌 legacy read-only 장부로 격리합니다.
- 활성 멤버가 두 명을 초과하거나 owner가 유일하지 않은 공동 장부는 자동으로 멤버를 제거하지 않고 legacy read-only로 격리합니다.
- 이전 구현에 남아 있지 않은 탈퇴자 이력은 추측해 만들지 않습니다. 기존 공동 장부의 재초대 제한은 확인 가능한 상대방 이력부터 적용합니다.

### 2. 확장 스키마 추가

- 사용자 닉네임 확정 시각과 시간대
- 멤버십 참여 구간과 종료 사유
- 예산 기간·배분·예비비 이전·대분류 예산
- 거래 차감 할당, 이체 하위 유형, 공개 상태와 카테고리·결제수단 snapshot
- 예약 계획·occurrence와 일시정지 사유
...
1. 모든 사용자에 기본 개인 장부를 하나 지정합니다. 없으면 생성하고 중복이면 기존 owner·최근 사용 이력을 기준으로 하나를 기본으로 표시하되 나머지는 격리합니다.
2. 장부별 기본 예산 기간 규칙을 달력 월(`1일`)로 설정하고 기존 월을 동일한 날짜 범위의 `BudgetPeriod`로 변환합니다.
3. 개인 장부 거래는 소유자 개인 할당, 공동 장부 거래는 기존 payer 개인 할당으로 연결합니다.
4. category group을 system group code에 mapping하고 거래에 category snapshot을 채웁니다.
5. 기존 반복 거래를 예약 계획·occurrence로 옮기고 generation 기록으로 중복 방지 key를 채웁니다.
6. 기존 카드 거래에 소유자에게만 보이는 결제수단 snapshot을 채웁니다.
7. 기존 알림은 원래 수신자와 읽음 상태를 유지하고 새 알림 유형으로 mapping할 수 없는 항목은 legacy type으로 보존합니다.

### 4. 이중 검증과 읽기 전환

- 장부·기간별 거래 건수와 수입·지출 합계 비교
- 기존 월 총예산과 새 기간 총예산 비교
- 사용자별 접근 가능한 장부와 거래 수 비교
- category group별 합계와 snapshot 집계 비교
- 예약 계획별 과거 생성 거래와 occurrence 비교
- 샘플 사용자 UI에서 개인·공동·비공개 거래 노출 확인
...
- `V8`~`V15`는 V1 확장 스키마를 추가하고, `V16__backfill_v1_read_models.sql`은 기존 월 예산·멤버 배분·거래·반복 거래·카드 표시값과 사용자별 기본 설정을 V1 read model로 옮깁니다.
- `V16`은 구형 테이블을 삭제하지 않으며 다시 실행되어도 동일 key를 중복 생성하지 않도록 작성합니다.
- 공동 장부의 구형 category budget은 개인/공동 귀속을 추측하지 않고 구형 테이블에 보존합니다. 첫 V1 예산 설정에서 사용자가 공동·개인 배분을 확정해야 합니다.
