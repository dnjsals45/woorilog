...
- 기본 개인 가계부는 사용자마다 정확히 하나이며 숨김·보관·삭제할 수 없습니다.
- 사용자가 접근할 수 없는 `lastUsedLedgerId`를 가리키면 기본 개인 가계부로 복구합니다.

### Ledger

- `PERSONAL` 또는 `SHARED` 타입을 가집니다.
- 개인 가계부는 소유자 한 명만 사용합니다. V1에서는 추가 개인 가계부를 만들 수 없습니다.
- 공동 가계부는 현재 활성 `OWNER` 한 명과 활성 `MEMBER` 최대 한 명을 가집니다.
- 공동 가계부는 이름, 예산 기간 규칙과 예산 기본값을 가집니다.
- V1에서는 공동 가계부 보관을 제공하지 않습니다. 삭제는 `ledgers.deleted_at` 으로 표시합니다. 삭제된 가계부의 초대 링크가 만료와 구분되려면 행이 남아 있어야 하기 때문입니다.
- 한 번이라도 참여한 상대방이 있으면 그 상대방만 같은 가계부에 재참여할 수 있습니다.

### Ledger Membership

- 사용자와 가계부의 참여 관계이며 `OWNER`, `MEMBER` 역할을 가집니다.
- `joinedAt`, `leftAt`, `leaveReason`으로 참여 구간을 보존합니다. 재참여하면 새 참여 구간을 추가합니다.
- 공동 가계부별 활성 멤버십은 최대 두 개이고 `OWNER`는 정확히 하나입니다.
- 탈퇴·내보내기 후에는 참여했던 예산 기간과 당시 보이던 데이터만 읽을 수 있습니다.
- 소유권 이전은 두 활성 멤버의 역할을 한 트랜잭션 안에서 교환합니다.
- 탈퇴 또는 내보내기는 가계부의 모든 활성 자동 기록를 일시정지하고 원인을 기록합니다.

### Ledger User Preference

- 사용자와 가계부별 빠른 기록 기본값을 저장합니다.
- `lastBudgetSource`, `shareNewPersonalTransactions`를 가지며 상대방 설정과 분리합니다.
- 마지막 차감 대상이 더 이상 유효하지 않으면 본인 개인 예산을 기본값으로 복구합니다.
- 전체 공유를 켤 때만 기존 개인 거래 보임 상태를 함께 변경하고, 끌 때는 이후 기본값만 변경합니다.

### Invitation

- 공동 가계부의 링크 초대만 표현합니다.
...
- 가계부마다 `PENDING` 링크는 하나뿐이며 생성 후 30분 동안 유효합니다.
...
- 같은 가계부의 기간은 겹치지 않으며 `(ledgerId, startDate)`가 유일합니다.
...
- 한 기간에 활성 멤버별 개인 예산 하나와 공동 예산 하나가 존재할 수 있습니다.
- 처음 나누기 전에는 allocation과 예비비가 아직 확정되지 않습니다. 나눈 뒤 `reserve = totalBudget - personal allocations - shared allocation`으로 계산하며 별도 직접 차감 대상이 아닙니다.
- 나눈 금액 합계가 전체 예산을 초과하면 사용자가 명시적으로 전체 예산 증액을 승인한 요청에서만 저장합니다.
- 두 활성 멤버는 상위 예산 금액을 조회·변경할 수 있지만 개인 예산에서 거래를 차감하는 권한은 해당 사용자에게만 있습니다.

### Reserve Transfer

- 예비비를 본인 개인 예산 또는 공동 예산으로 옮긴 기록입니다.
- `amount`, `targetAllocationId`, `actorUserId`, `createdAt`을 가집니다.
- 원본 금액을 직접 수정하지 않고 이전 기록의 합으로 현재 나눈 금액을 설명할 수 있어야 합니다.
- 두 멤버 모두 조회하며 취소가 필요하면 반대 방향의 보정 기록을 추가합니다.

### Category Budget

- `BudgetAllocation`과 고정 카테고리 조합의 세부 예산입니다.
- 공동 예산의 카테고리 예산은 두 멤버가 조회·변경합니다.
- 개인 예산의 카테고리 예산은 대상 사용자만 조회·변경합니다.
- 합계가 상위 예산보다 작을 수 있으며 차이는 남은 금액입니다.
- 상위 예산보다 크게 저장하려면 같은 요청에서 상위 예산 증액을 명시적으로 승인해야 합니다.

## 카테고리

### System Category Group

- 시스템이 관리하는 변경 불가능한 카테고리입니다.
- 거래 유형과 안정적인 code, 표시 이름, 정렬 순서를 가집니다.
- V1의 고정 목록과 기본 세부 카테고리는 [V1 Scope의 카테고리 구조](../product/v1-scope.md#카테고리-구조)를 원본으로 사용합니다.
- 사용자는 가계부별로 카테고리를 숨길 수 있지만 생성·이름 변경·삭제할 수 없습니다.

### Ledger Category

- 한 가계부의 세부 카테고리입니다.
- 시스템 카테고리 아래에 속하며 이름, 활성 여부, 기본 카테고리 여부를 가집니다.
- 같은 가계부와 카테고리 안에서 활성 이름은 중복될 수 없습니다.
- 기본 세부 카테고리도 사용자 생성 세부 카테고리와 같이 이름 변경·삭제할 수 있습니다.
- 삭제는 새 거래에서 선택할 수 없게 하는 비활성화이며 과거 거래 스냅샷은 유지합니다.

### Transaction Category Snapshot

- 거래 저장 시점의 카테고리 code·이름과 세부 카테고리 id·이름을 보존합니다.
- 미분류 거래는 세부 카테고리 id와 이름이 `null`이고 카테고리 집계에서 `UNCLASSIFIED`로 분리합니다.
- 세부 카테고리 이름 변경은 기본적으로 이후 거래에만 적용합니다.
- `과거 거래에도 적용` 요청은 접근 가능한 해당 가계부 거래의 스냅샷을 명시적으로 일괄 변경합니다.
...
- 금액, 사용처, 발생일, 선택 시각, 메모, 기록자, 결제자, 결제 수단 정보를 가집니다.
- 공동 가계부 거래는 `PERSONAL` 또는 `SHARED` scope를 가집니다. 개인 scope는 소유 사용자 id를 함께 저장합니다.
- 지출과 지출로 처리하는 이체는 `BudgetAllocation`을 반드시 참조합니다.
- 수입과 `OWN_ACCOUNTS`, `INBOUND` 이체는 예산을 참조하지 않습니다.
...
- 결제자와 결제 수단은 설명 정보이며 차감할 예산을 결정하지 않습니다.
- 공동 예산 거래는 두 멤버가 조회·수정·삭제할 수 있습니다.
- 개인 예산 거래는 대상 사용자만 수정·삭제하고, 상대방은 공유된 거래만 읽을 수 있습니다.
- 공동 거래 수정 시 `lastModifiedBy`, `lastModifiedAt`을 갱신합니다.
- 빠른 기록은 카테고리가 필수이고 가져오기 저장은 미분류를 허용합니다.

### Transaction Visibility

- 공동 가계부의 개인 scope 거래는 `sharedWithPartner`를 가지며 기본값은 기록자의 가계부별 공유 기본 설정입니다.
- 공유 기본값을 켜면 기존 개인 거래를 모두 공유하고 이후 거래의 기본값도 `true`가 됩니다.
- 공유 기본값을 끌 때는 기존 거래를 바꾸지 않고 이후 기본값만 `false`로 바꿉니다.
- 거래별 보임 설정은 공유 기본값보다 우선합니다.
- 공유된 개인 거래에서도 카드 id·카드 이름 등 결제 수단 식별 정보는 상대방에게 반환하지 않습니다.

### Payment Method Snapshot

- 거래에는 `CASH`, `CARD`, `OTHER` 같은 결제 수단 종류와 선택적인 표시 스냅샷을 저장할 수 있습니다.
- 카드 식별 정보는 결제한 사용자 본인에게만 보입니다.
- V1은 별도 카드 관리 화면을 범위에 포함하지 않으므로 거래 계약이 카드 원장 관리에 의존하지 않게 합니다.

### Installment Plan

- 최초 지출에서 전체 원금, 회차 수, 월 이자, 첫 결제일과 차감 예산을 저장합니다.
...
- 자동 기록 계획의 특정 예정일과 생성된 거래를 연결합니다.
...
- 한 사용자가 한 가계부에 업로드한 여러 이미지의 검토 세션입니다.
...
- 추천 카테고리, 차감 예산, 중복 의심 여부·근거, 선택 기본값을 가집니다.
- 공동 가계부 후보의 기본 차감 대상은 업로드한 사용자의 개인 예산입니다.
- 중복 후보는 자동 삭제하지 않고 기본 선택만 해제합니다.
- 카테고리 추천 이력은 사용자와 가계부를 함께 범위로 사용합니다.
...
- 현재 또는 선택한 `BudgetPeriod`의 공동 예산과 로그인 사용자의 개인 예산만 홈에 표시합니다.
...
- 상세 조회에서 상대방 예산의 합계는 볼 수 있지만 개별 거래는 함께 보기로 한 거래만 반환합니다.

### Notification

- 사용자별 앱 내부 알림과 읽음 상태를 저장합니다.
- 알림은 관련 가계부와 선택적인 예산 기간·거래·설정 화면의 이동 대상을 가집니다.
- 예산 임계값, 예산 변경, 예비비 이전, 다음 기간 준비, 멤버 내보내기, 자동 기록 일시정지와 주간 가이드를 구분합니다.
- 예산 기간에 속한 알림은 해당 가계부의 다음 기간이 시작될 때 사용자 목록에서 제거합니다.

### Budget Threshold State

- 예산 또는 카테고리 예산별 현재 알림 단계 `BELOW_80`, `AT_LEAST_80`, `AT_LEAST_100`을 저장합니다.
...
- 사용자, 가계부, 생성 주와 예산 기간별 권장액 스냅샷입니다.
- 공동 예산과 본인 개인 예산만 계산하며 상대방 개인 예산은 포함하지 않습니다.
- 같은 사용자·가계부·주에 중복 생성하지 않습니다.
- 계산식과 시간 규칙은 [Scheduled Transactions](./scheduled-transactions.md)를 따릅니다.

## 핵심 불변식

- 사용자마다 기본 개인 가계부는 정확히 하나입니다.
- 공동 가계부의 활성 멤버는 최대 두 명이고 `OWNER`는 정확히 한 명입니다.
- 가계부의 예산 기간은 겹치지 않으며 과거 기간 경계는 정책 변경으로 움직이지 않습니다.
- 예비비는 직접 거래 차감 대상이 아닙니다.
- 사용자는 본인 개인 예산과 공동 예산에만 거래를 기록할 수 있습니다.
- 상대방이 나만 보는 개인 거래는 금액 합계 API를 제외한 거래 상세·최근 거래·분석·검색 결과에 노출하지 않습니다. 상대방 예산 합계는 V1 Scope가 허용한 상세 화면에서만 제공합니다.
...
- 결제 수단 식별 정보는 공유된 개인 거래에서도 결제한 사용자 외에는 보이지 않습니다.
