...
- 최초 Kakao 로그인은 인증과 기본 개인 가계부 생성을 완료한 뒤 `nicknameConfirmed=false`를 반환합니다. 사용자는 제품 쓰기 전에 서비스 닉네임을 확정합니다.
- 일반 로그인은 닉네임 확정 후 기본 개인 가계부로, 초대 링크에서 시작한 로그인은 닉네임 확정 후 원래 초대 확인 경로로 돌아갑니다.
...
- dev login은 `DEV` provider user를 upsert하고, 접근 가능한 가계부가 없으면 기본 개인 가계부를 생성합니다.
...
- refresh 후에도 사용자와 현재 가계부, 닉네임 확정 상태를 함께 반환해 새로고침 경로가 같은 분기를 재현하게 합니다.
