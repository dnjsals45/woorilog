...
| 우선순위 | 위험 | 주 검증 |
| --- | --- | --- |
| P0 | 상대방이 나만 보는 거래·카드 식별 정보 노출 | API integration, E2E |
...
| P1 | 탈퇴 후 권한과 자동 기록 상태 오류 | integration |
...
- category budget의 남은 금액·상위 예산 증액 확인
...
- resource id API가 가계부 멤버십을 다시 검사하는지
...
- 사용자별 기본 개인 가계부 exactly-one 보장
- 공동 가계부 owner 1명·활성 멤버 최대 2명 constraint
- 초대 링크 교체·30분 만료·accept/reject·동시 수락
- 소유권 이전, 탈퇴·내보내기, 재참여와 다른 상대방 차단
- 멤버십 종료 시 자동 기록 계획 전체 일시정지
...
- import preview의 저신뢰 제외, 사용자·가계부별 추천, batch save 원자성·멱등성
- notification threshold 재진입, read-all과 기간 전환 정리
- migration 전후 가계부·거래·금액 합계와 권한 query
...
- 가계부 선택과 former read-only 상태
- 초대 전·예산 전·거래 전 dashboard empty state
- 현재/과거 예산 기간 이동과 말일 표시
- 개인·공동·예비비 나누기, 증액 확인과 저장 실패 feedback
- 빠른 기록의 필수값·기본 차감 대상·저장 후 계속 입력
- 거래 검색·복합 filter·미분류 일괄 처리
- 전체 공유 on/off와 거래별 보임 설정
- import 후보 기본 선택, 중복 강조, 일괄 category/source 변경과 제외 건수
- 자동 기록 pause/resume과 고정비·할부 표시
- 알림 개별/전체 읽음과 설정 가능한 알림만 노출
- loading, empty, error, retry와 mutation 실패 상태

API mock은 [API Contract](./api-contract.md)의 실제 response shape를 사용하고 상대방에게 보이지 않는 필드를 편의상 추가하지 않습니다.

### Browser E2E

핵심 경로:

1. 신규 login → nickname 확정 → 기본 개인 가계부 첫 거래
2. 공동 가계부 생성 → 링크 공유 → 다른 사용자 login → 명시적 수락
3. 두 사용자 예산 나누기 → 본인·공동 거래 → dashboard 합계
4. 개인 거래 공유 변경 → 상대방 상세 노출과 home/analytics 제외 확인
5. 여러 이미지 preview → 중복 후보 제외 → 선택 batch 저장
6. member 탈퇴/내보내기 → read-only 과거 조회와 자동 기록 pause
...
- 긴 가계부명·닉네임, 큰 금액, 빈/많은 거래와 category
...
- fixture 이름과 금액은 제품 용어인 개인 예산, 공동 예산, 예비비와 예산 기간을 사용합니다.
