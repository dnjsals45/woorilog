...
이 문서는 할부, 반복 지출, 고정비와 주간 예산 가이드의 예약 실행 기준을 정의합니다.

## 공통 원칙

- 모든 예약 실행은 중복 방지 key를 가지고 재시도에 안전해야 합니다.
- 날짜 계산은 장부 예산 기간, 실행 시각은 사용자 시간대를 사용합니다.
...
- 멤버십 변경으로 멈춘 계획은 남은 사용자가 접근 가능한 본인 또는 공동 할당으로 대상이 유효한지 확인한 뒤 재개합니다.
...
- 기간 시작 시 다음 설정이 없으면 직전 기간의 전체·개인·공동·대분류 예산 설정을 복사합니다.
- 잔액과 초과액은 복사하지 않습니다.
- 같은 장부와 기간에 작업이 재실행돼도 `BudgetPeriod`와 알림을 중복 생성하지 않습니다.
