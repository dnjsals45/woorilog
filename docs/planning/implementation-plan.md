...
- 직접 사용자 검색 초대 제거
- 공동 장부 보관 진입점 제거 (삭제는 2026-08-03 결정으로 V1 범위에 다시 들어왔습니다)
- 수동 월 마감·재오픈 화면 제거
- 달력 전용 거래 탐색을 날짜순 목록으로 전환
- 카드 관리와 정산 등 V1에 없는 화면·경로의 처리
- 주 이동을 홈·가계부·반복 거래·예산 설정·분석 다섯으로 전환 (V1은 데스크톱 폭 전용)
...
| 반복 거래 적용 범위 `ALL` | `ScheduledPlanService.updateFuture` 가 `FUTURE` 만 지원합니다 | 화면에서 선택지를 없애 지금은 드러나지 않습니다 |

### 남은 제품 판단

| 항목 | 내용 |
| --- | --- |
| 페어링 제한 완화 | 한 번이라도 두 사람이 쓴 장부는 원래 상대방만 다시 참여할 수 있습니다. 떠난 사람의 기록을 지키려는 규칙인데, **잘못 초대해서 바로 내보낸 장부도 영영 잠깁니다.** 그 경우엔 지킬 기록이 없습니다. 실제로 이 일이 생기면 "거래를 남기지 않고 나간 멤버는 이력으로 치지 않는다" 같은 완화를 검토합니다. 지금은 혼자 남은 장부를 삭제할 수 있어 탈출구가 있습니다. |
