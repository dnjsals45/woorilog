...
- [Domain Model](../engineering/domain-model.md), [API Contract](../engineering/api-contract.md), 권한·개인정보·migration·OCR·자동 기록 실행 문서는 새 V1 목표 계약으로 교체되었습니다.
...
- 확정 디자인(Crisp Calm V1) 이식과 그 과정에서 드러난 불일치·API 공백은 모두 정리되었습니다.
  결정 내용은 각 상시 문서에 반영했고, 남은 항목만 아래 "확정 디자인 이식 결과"에 적어 둡니다.
...
- 공동·개인 예산 및 거래가 보이는 범위의 권한 표
- 기존 가계부 보관, 직접 초대, 월 마감, 정산과 카드 관리 기능의 유지·제거 방식
...
- [x] [Permissions](../engineering/permissions.md)와 [Privacy](../engineering/privacy.md)에 두 사용자 권한과 보이는 범위가 정리되어 있습니다.
- [x] [Data Migration](../engineering/data-migration.md)에 기존 데이터의 보존·변환·제거 기준이 정리되어 있습니다.
- [x] [Transaction Import](../engineering/transaction-import.md)와 [Scheduled Transactions](../engineering/scheduled-transactions.md)에 OCR·자동 기록 실행 경계가 정리되어 있습니다.
...
- 최초 닉네임 확정과 기본 개인 가계부
- 최대 두 명의 공동 가계부
- 가계부 이름·전체 예산·예산 기간을 포함한 생성
...
- 신규 사용자가 개인 가계부로 시작할 수 있습니다.
- 사용자가 공동 가계부를 만들고 링크로 한 명을 초대할 수 있습니다.
- 탈퇴·내보내기 후 데이터가 보이는 범위와 자동 거래 일시정지가 정책대로 동작합니다.
...
- 고정 카테고리, 가계부별 세부 카테고리와 거래 카테고리 스냅샷
- 날짜순 목록, 검색·필터, 미분류 일괄 처리

완료 기준:

- 사용자가 권한 범위 안에서 거래를 생성·수정·삭제할 수 있습니다.
- 세부 카테고리를 변경하거나 삭제해도 과거 거래와 카테고리 분석이 유지됩니다.
- 상대방이 나만 보는 거래가 홈·분석·상세 API에서 노출되지 않습니다.
...
## 5. 자동 기록 — 반복 지출·고정비·할부

범위:

- 할부 원금, 개월 수, 월 이자와 회차별 자동 기록
- 매주·매월·매년 반복 지출
- 존재하지 않는 월 일자의 말일 보정
- 선택형 고정비 관리
...
- 주간 가이드, 수입 합계와 최근 거래
- 예산 상세와 지난 기간 조회
- 카테고리 원형 그래프, 일별·누적 흐름과 이전 기간 비교
- 6·12개 기간 추세
...
- 다음 기간 3일 전 설정 안내
- 일요일 오후 9시 주간 권장액
- 기간 전환 시 이전 가계부 알림 제거

완료 기준:

- 공동·본인·카테고리 예산의 수신자가 보이는 범위와 일치합니다.
...
- 직접 사용자 검색 초대 제거
- 공동 가계부 보관 진입점 제거 (삭제는 2026-08-03 결정으로 V1 범위에 다시 들어왔습니다)
- 수동 월 마감·재오픈 화면 제거
- 달력 전용 거래 탐색을 날짜순 목록으로 전환
- 카드 관리와 정산 등 V1에 없는 화면·경로의 처리
- 주 이동을 홈·거래 내역·자동 기록·예산 설정·분석 다섯으로 전환 (V1은 데스크톱 폭 전용)
...
Crisp Calm V1 디자인 14화면 이식과 그에 따른 백엔드 보완은 끝났습니다(2026-08-02 ~ 08-03).
결정 사항은 `v1-scope.md`, `permissions.md`, `docs/design/**`, `api-contract.md`에 반영했습니다.
여기에는 **아직 남은 것만** 적습니다.
...
| 항목 | 내용 | 영향 |
| --- | --- | --- |
| 기간별 고정비·할부 항목 목록 | 총액(`scheduledRecurringExpenseAmount`, `nextPeriodScheduledAmount`)만 있고 항목별 목록이 없습니다 | 대시보드 예산 상세와 기간 종료 요약의 해당 영역이 빈 상태입니다. `ScheduledPlan` 응답에 필요한 필드는 이미 있으므로 목록 엔드포인트나 요약 응답 확장으로 풀 수 있습니다 |
| 자동 기록 적용 범위 `ALL` | `ScheduledPlanService.updateFuture` 가 `FUTURE` 만 지원합니다 | 화면에서 선택지를 없애 지금은 드러나지 않습니다 |

### 남은 제품 판단

| 항목 | 내용 |
| --- | --- |
| 페어링 제한 완화 | 한 번이라도 두 사람이 쓴 가계부는 원래 상대방만 다시 참여할 수 있습니다. 떠난 사람의 기록을 지키려는 규칙인데, **잘못 초대해서 바로 내보낸 가계부도 영영 잠깁니다.** 그 경우엔 지킬 기록이 없습니다. 실제로 이 일이 생기면 "거래를 남기지 않고 나간 멤버는 이력으로 치지 않는다" 같은 완화를 검토합니다. 지금은 혼자 남은 가계부를 삭제할 수 있어 탈출구가 있습니다. |

### 남은 에셋

랜딩 기능 섹션의 16:10 사진(일요일 저녁 집 안 분위기)이 아직 없어 자리표시자입니다.
5:3 영수증 사진은 넣었지만 영문·달러 표기라 한글 영수증으로 교체할 예정입니다.

### 이식에서 얻은 것

백엔드에 legacy 계층과 V1 계층이 쌍으로 공존합니다(`NotificationResponse` ↔ `V1NotificationResponse` 등).
처음 API 공백 목록을 만들 때 **프론트 클라이언트의 부재를 백엔드의 부재로 오인해 13개 중 4개를 잘못 판단했습니다.**
API 공백을 판단할 때는 컨트롤러와 응답 DTO를 직접 확인합니다.

프론트 API 타입의 필드명이 실제 응답 키와 달라 런타임에 `undefined`가 되는 사례가 6건 있었습니다.
`tsc`도 단위 테스트도 잡지 못합니다 — 프론트 타입끼리는 일관되고 실제 응답을 태우는 테스트가 없기 때문입니다.
자세한 내용과 대책 검토는 `docs/engineering/testing-strategy.md`에 있습니다.
