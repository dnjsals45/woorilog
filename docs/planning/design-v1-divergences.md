...
| 항목 | 내용 |
| --- | --- |
| 예비비 이동과 예산 배분의 비일관성 | 예산 배분에서는 상대방 할당액을 직접 입력할 수 있는데, 예비비는 상대방 예산으로 옮길 수 없습니다(위 2번). 기간 예산을 함께 짜는 것과 기간 중에 상대 예산 잔액을 바꾸는 것은 다른 행동이라고 보고 지금은 그대로 뒀습니다. 사용자에게 일관성 없이 보일 수 있어 판단이 필요합니다. |
