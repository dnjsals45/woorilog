...
| 항목 | 내용 |
| --- | --- |
| `DIFFERENT_PARTNER_NOT_ALLOWED` 대응 화면 | 상대가 나간 뒤 owner 가 새 링크를 만들어도 제3자는 참여할 수 없습니다(탈퇴 이력까지 포함해 판정). 의도된 페어링 보호로 보이지만 디자인에 대응 화면이 없어 **만료 화면으로 폴백** 중입니다. 전용 화면이 필요한지 판단이 필요합니다. |
| 카드 연결 | 거래 저장 시 카드를 고르면 **이름만** 들어가고 `cardId` 는 들어가지 않습니다. V1 저장 API 에 필드가 없습니다. 대시보드에 `cardPaymentSummaries` 가 이미 있어 카드별 집계와 이어지려면 백엔드 필드가 필요합니다. |
| 사진 에셋 2장 | 위 11번. |
