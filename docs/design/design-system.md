...
| --- | --- | --- | --- |
| Page title | `23-30px` | `700` | 현재 가계부명, 화면 제목 |
...
- 주요 영역은 `홈 / 거래 내역 / 자동 기록 / 예산 설정 / 분석` 5개입니다.
...
- **Switch(토글)** — 자동 기록 화면(고정비 표시)과 설정 알림 탭이 각각 인라인으로 구현합니다.
...
- 가로 스크롤, 고정 요소 겹침, 긴 가계부명과 큰 금액의 잘림이 없어야 합니다.
...
- 모바일에서는 현재 가계부명을 화면 제목과 결합한 44px 선택 영역으로 제공합니다.
