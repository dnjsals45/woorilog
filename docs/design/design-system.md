...
| `data-violet` | `#8A6EDB` | 정산 또는 네 번째 비교 계열 |
...
- 지출·수입·예산·정산 금액의 기본 텍스트는 `ink`입니다.
...
- 순서는 `홈 / 거래 / 기록 / 예산 / 분석`입니다.
- `기록`은 중앙 항목에 통합하고, 콘텐츠 위에 별도 확장형 FAB를 중복 배치하지 않습니다.
