package com.woorilog.application.invitation.service
...
import com.woorilog.application.invitation.result.toResult
...
class InvitationService(
    ...
    fun createV1InvitationLink(currentUserId: Long, ledgerId: Long): V1InvitationLinkCreatedResult {
        ...
        return V1InvitationLinkCreatedResult(invitation.id!!, "/invitations/$rawToken", invitation.expiresAt!!)
    }
    ...
    fun acceptV1Link(currentUserId: Long, rawToken: String): V1InvitationAcceptedResult {
        ...
        return V1InvitationAcceptedResult(
            LedgerSummaryResult(
                id = ledger.id!!,
                name = ledger.name,
                type = "SHARED",
    ...
    fun inviteUser(currentUserId: Long, ledgerId: Long, targetUserId: Long): InvitationResult {
        ...
        val currentUser = userRepository.findByIdOrNull(currentUserId) ?: throw ForbiddenException("사용자를 찾을 수 없습니다.")
        ...
        return saved.toResult(clock.instant())
    }
    ...
    fun getLedgerInvitations(currentUserId: Long, ledgerId: Long): List<InvitationResult> {
        ...
        return list.map { it.toResult(now) }
    }
    ...
    @Transactional(readOnly = true)
    fun getPendingInvitations(currentUserId: Long): List<InvitationResult> {
        ...
        val now = clock.instant()
        return list.filter { !it.isExpired(now) }.map { it.toResult(now) }
    }

    fun acceptInvitation(currentUserId: Long, invitationId: Long): InvitationResult {
        val invitation = invitationRepository.findByIdOrNull(invitationId) ?: throw NotFoundException("초대를 찾을 수 없습니다.")
    ...
    fun rejectInvitation(currentUserId: Long, invitationId: Long): InvitationResult {
        val invitation = invitationRepository.findByIdOrNull(invitationId) ?: throw NotFoundException("초대를 찾을 수 없습니다.")
    ...
    @Transactional(readOnly = true)
    fun getLinkInvitationPreview(token: String): LinkInvitationPreviewResult {
        val invitation = invitationRepository.findByToken(token)
            ?: throw NotFoundException("초대 링크를 찾을 수 없습니다.")

        return LinkInvitationPreviewResult(
    ...
    fun acceptLinkInvitationByToken(currentUserId: Long, token: String): InvitationResult {
        val invitation = invitationRepository.findByToken(token)
            ?: throw NotFoundException("초대 링크를 찾을 수 없습니다.")
