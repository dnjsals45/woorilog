...
class BudgetPeriodService(
    ...
    fun configure(
        ...
        resolvedPersonalInputs.forEach { input ->
            val owner = userRepository.findById(input.userId).orElseThrow { NotFoundException("사용자를 찾을 수 없습니다.") }
    ...
    fun transferReserve(
        ...
        val targetAllocation = allocations.firstOrNull {
            when (target.type) {
                "SHARED" -> it.scope == BudgetAllocationScope.SHARED && target.ownerUserId == null
                "PERSONAL" -> it.scope == BudgetAllocationScope.PERSONAL &&
                    it.owner?.id == userId && target.ownerUserId == userId
                else -> false
            }
    ...
    private fun createPeriodForDate(ledger: Ledger, date: LocalDate): BudgetPeriod {
        ...
        if (ledger.type == LedgerType.PERSONAL) {
            val owner = userRepository.findById(ledger.ownerId).orElseThrow { NotFoundException("사용자를 찾을 수 없습니다.") }
            budgetAllocationRepository.save(
